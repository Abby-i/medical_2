...
　　　　　　 title: '慢病报销汇款审核',　　
 　　　　　　content: 'system/RemittanceExpenseServlet?m=get&id='+id,　// 要跳转的页面地址（跟超链接是一样的形式，也可拼接赋值）
...
<body>
	<form method="post" action="<%=path %>/system/RemittanceExpenseServlet?m=list" id="listform">
		<div class="panel admin-panel">
			...
			<div class="padding border-bottom">
				<ul class="search" style="padding-left:10px;">
					...
					<li>
						<div class="field">
						    <select id="remittanceStatus" name="remittanceStatus" class="input" >
						  		<option value=""></option>
...
<%
 ...
 %>				
				...
				<tr>
					...
					<c:if test="${not empty F020602 }">		
							 <a class="button border-red"
								href="<%=path%>/system/RemittanceExpenseServlet?m=del&id=<%=model.getId() %>" onclick="del()"><span
			...
			<div class="pagelist">
	    			...
	    			<c:otherwise>
	    					<a href="${pageContext.request.contextPath }/system/RemittanceExpenseServlet?m=list&currentPage=${pageBean.firstPage }&pageSize=${pageBean.pageSize}">首页</a> 
							<a href="${pageContext.request.contextPath }/system/RemittanceExpenseServlet?m=list&currentPage=${pageBean.prePage }&pageSize=${pageBean.pageSize}">上一页</a> 
	    			</c:otherwise>
	    			</c:choose>
	    			...
	    			<c:choose>
						...
						<c:otherwise>
							<a href="${pageContext.request.contextPath }/system/RemittanceExpenseServlet?m=list&currentPage=${pageBean.nextPage }&pageSize=${pageBean.pageSize}">下一页</a>
							<a href="${pageContext.request.contextPath }/system/RemittanceExpenseServlet?m=list&currentPage=${pageBean.totalPage }&pageSize=${pageBean.pageSize}">尾页</a>
							</c:otherwise>   		
    				</c:choose>
...
<script type="text/javascript">
  //1.改变每页显示记录数
  	function changePageSizer(){
  		...
  		var url = "${pageContext.request.contextPath}/system/RemittanceExpenseServlet?m=list&pageSize="+pageSize;
  	...
  	function changePager(){
  		...
  		var url="${pageContext.request.contextPath}/system/RemittanceExpenseServlet?m=list&currentPage="+curPage+"&pageSize=${pageBean.pageSize}";
  		window.location.href=url;
  	}
  	...
  	function editUser(id){
		$.post('/system/RemittanceExpenseServlet?m=get?'+id,function() {
			location.reload();
		});
	}
