...
%>
<%
	MedicalExpenseDao illExpenseDao = new MedicalExpenseDao();
...
<script type="text/javascript">
	//三级联动
	$(function(){
	     		$.get("AreaServlet?action=tojson",{areaupcode:450421},function(data){
	     			//使用eval方法将数据转换为js对象
	     				eval("var areas="+data);
	     				//获取下拉框对象
	     				var sel=$("#town");
	     				...
	     				sel.empty();
	     				//遍历
	     				sel.append("<option value=''>"+"默认"+"</option>");
     			...
     			$("#town").change(function(){
     				...
     				//发起ajax请求，请求当前省的市信息
     				$.get("AreaServlet?action=tojson",{areaupcode:areacode},function(data){
     					//使用eval方法将数据转换为js对象
     					eval("var areas="+data);
     					//获取下拉框对象
     					var sel=$("#village");
...
　　　　　　 title: '查看缴费标准',　　
 　　　　　　content: 'CheckExpenseServlet?action=get&id='+id,　// 要跳转的页面地址（跟超链接是一样的形式，也可拼接赋值）
...
<body>
	<form method="post" action="<%=path %>/CheckExpenseServlet?action=list" id="listform">
		<div class="panel admin-panel">
			...
			<div class="padding border-bottom">
				<ul class="search" style="padding-left:10px;">
					...
					<li>
						<div class="field">
							<select id="village" name="village" class="input">
					...
					<li>
						<div class="field">
						    <select id="auditStatus" name="auditStatus" class="input" >
			...
			<table class="table table-hover text-center">
				<tr>	
				    ...
				    <th>操作员</th>
				    <th>审批人</th>
				    <c:if test="${not empty F020601 && not empty F020602}">		
					<th>操作</th>
					</c:if>
				</tr>
<%
	List illExpenses =(List) session.getAttribute("illExpenses");
	Iterator iter = illExpenses.iterator();
	int i = 0;
	while (iter.hasNext()) {
	MedicalExpense model = (MedicalExpense)iter.next();
 %>				
				...
				<tr>
					...
					<c:if test="${not empty F020601 }">		
							<a class="button border-main"
								href="javascript:void(0)" onclick="openAudit(<%=model.getId()%>)"><span
								class="icon-edit"></span> 审核</a>
					</c:if>
					<c:if test="${not empty F020602 }">		
							 <a class="button border-red"
								href="<%=path%>/CheckExpenseServlet?action=del&id=<%=model.getId() %>" onclick="del()"><span
			...
			<div class="pagelist">
	    			...
	    			<c:otherwise>
	    					<a href="${pageContext.request.contextPath }/CheckExpenseServlet?action=list&currentPage=${page.firstPage }&pageSize=${page.pageSize}">首页</a> 
							<a href="${pageContext.request.contextPath }/CheckExpenseServlet?action=list&currentPage=${page.prePage }&pageSize=${page.pageSize}">上一页</a> 
	    			</c:otherwise>
	    			</c:choose>
	    			...
	    			<c:choose>
						...
						<c:otherwise>
							<a href="${pageContext.request.contextPath }/CheckExpenseServlet?action=list&currentPage=${page.nextPage }&pageSize=${page.pageSize}">下一页</a>
							<a href="${pageContext.request.contextPath }/CheckExpenseServlet?action=list&currentPage=${page.totalPage }&pageSize=${page.pageSize}">尾页</a>
							</c:otherwise>   		
    				</c:choose>
							当前为第${page.currentPage }页/共${page.totalPage } 页&nbsp;
				    		共${page.totalCount }条数据&nbsp;
				    		每页显示<input type="text" size="2" id="pageSize" value="${page.pageSize }" onblur="changePageSizer()">条
...
<script type="text/javascript">
  //1.改变每页显示记录数
  	function changePageSizer(){
  		...
  		var url = "${pageContext.request.contextPath}/CheckExpenseServlet?action=list&pageSize="+pageSize;
  	...
  	function changePager(){
  		...
  		var url="${pageContext.request.contextPath}/CheckExpenseServlet?action=list&currentPage="+curPage+"&pageSize=${page.pageSize}";
  		window.location.href=url;
  	}
  	...
  	function editUser(id){
		$.post('/CheckExpenseServlet?action=get?'+id,function() {
			location.reload();
		});
	}
